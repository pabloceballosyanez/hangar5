...
export default function WaiterOrderPage() {
  ...
  const tabSessionId = params.sessionId as string;
    ...
    const load = async () => {
      try {
        const [menuRes, sessRes] = await Promise.all([
          ...
          fetch(`/api/admin/restaurant/sessions/${tabSessionId}`),
        ]);
        ...
        const sessData: { id: string; status: string; label: string } = await sessRes.json();

        if (sessData.status !== 'OPEN') {
          router.replace(`/waiter/session/${tabSessionId}`);
          return;
        }
  ...
  const sendToKitchen = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setToast(null);
        router.push(`/waiter/session/${tabSessionId}`);
      }, 1800);
    } catch (e) {
  ...
  }, [currentSessionId, cart, tabSessionId, router]);
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <button onClick={() => router.push(`/waiter/session/${tabSessionId}`)} className="mt-2 px-6 py-3 bg-amber-500 text-slate-900 font-bold rounded-xl active:scale-95 transition-all">
          Volver
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-950 pb-32">
      ...
      <header className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur border-b border-slate-800">
        <div className="flex items-center gap-3 px-4 py-3">
          <button
            onClick={() => router.push(`/waiter/session/${tabSessionId}`)}
